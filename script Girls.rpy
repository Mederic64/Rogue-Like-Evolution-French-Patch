init python:
    ...
    class GirlClass(object):      
        def __init__(self,Name="no name",Love=0,Obed=0,Inbt=0,Lust=0):
                self.Name = __(Name)        #changable by player, used in dialog
                self.Tag = Name         #Permanent label, used in code
                self.Names = [__(Name)]     #this is a list of primary names you're allowed to use
                self.Love = Love
                self.Obed = Obed
                self.Inbt = Inbt
                self.Lust = Lust
        ...
        def OutfitChange(self, OutfitTemp = 0, Spunk = 0, Undressed = 0, Changed = 1,HolderOutfit=[]):
                ...
                if self.Loc == bg_current and renpy.showing("NightMask", layer='nightmask') and Current_Time == __("Morning"):
            ...
            menu:
                    ...
                    "Other":
                        menu:
                            ...
                            "When teaching you should wear. . . (locked)" if Girl == EmmaX and Cnt < 3:
                                pass
                            "When teaching you should wear. . ." if  Girl == EmmaX and Cnt >= 3:
                                call Clothes_ScheduleB(90)
                                $ Girl.Schedule[8] = _return 
...
label OutfitShame(Girl=0, Custom = 3, Check = 0, Count = 0, Tempshame = 50, Agree = 1):
        ...
        if not Check and not Girl.Taboo and Custom != 20:
                ...
                if Girl.Clothing[9] and "locked" not in Player.Traits:
...
label Bottoms_Off(Girl=0,Intro = 1, Line = 0, Cnt = 0):    
        ...
        if Approval >= 2:                
            ...
            if Girl == RogueX:            
                    if Approval >= 3:
                        $ Line = __("Hmmm, what do you want to see? . .")
                    else:    
                        $ Line = __("Well, ok. I'd kinda like to keep {i}some{/i} modesty though. . .")
            elif Girl == KittyX:
                    if Approval >= 3:
                        $ Line = __("Heh, what would you like to see? . .")
                    else:    
                        $ Line = __("Ok, maybe, but don't push it. . .")
            elif Girl == EmmaX:    
                    if Approval >= 3:
                        $ Line = __("Mmmm, what would you like?")
                    else:    
                        $ Line = __("What would you have me take off?")
            elif Girl == LauraX: 
                    if Approval >= 3:
                        $ Line = __("What did you want off?")
                    else:    
                        $ Line = __("Hm, what did you want me to lose?")
            ...
            menu:            
                ...
                "Come on, Please?":   
                        if "no bottomless" in Girl.DailyActions:    
                            ...
                            if Approval and ApprovalCheck(Girl, 600, "L", TabM=1):   
                                ...
                                if Girl == RogueX:
                                        $ Line = __("Well, what were you thinking then. . .")
                                elif Girl == KittyX:
                                        $ Line = __("I guess. . .")
                                elif Girl == EmmaX:    
                                        $ Line = __("Perhaps. . .")
                ...
                "It doesn't have to be everything. . ." if Girl.Legs or Girl.HoseNum() >= 10 or Girl.Panties == "shorts":    
                    if Approval and "no bottomless" not in Girl.DailyActions:                   
                            $ Girl.FaceChange("bemused", 1)
                            $ Line = __("Well what did you have in mind then?")
                            call Bottoms_Off_Legs(Girl)
                    else:    
...
label Bottoms_Off_Legs(Girl=0):     
        ...
        $ Line = __("Well what did you want off?") if not Line else Line
        $ Cnt = 1
