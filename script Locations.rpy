...
# Player Room Menu Start <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    menu:
        ...
        "Sleep" if Current_Time == __("Night"):            
                    call Round10
                    call Girls_Location
                    call EventCalls  
        "Wait" if Current_Time != __("Night"):
...
label Campus:
    ...
    call Checkout(1)    
    call GirlsAngry    
    if Current_Time == __("Evening") and "yesdate" in Player.DailyActions:
...
label Class_Room:    
    ...
    if "goto" in Player.RecentActions or "traveling" in Player.RecentActions:
            $ Present = []        
            if Current_Time != __(__("Night")) and Current_Time != __("Evening") and Weekday < 5:   
    ...
    if Line == __("entry"):
            if EmmaX.Loc == "bg teacher":
                    $ Line = __("As you sit down, you see "+ EmmaX.Name +" at the podium. What would you like to do?")
            elif Current_Time == __("Evening") or Weekday > 5:   
                    $ Line = __("You enter the classroom. What would you like to do?")
            else:
                    $ Line = __("You sit down at a desk. What would you like to do?")
    else:
            if Line != "What would you like to do next?":
                    $ Line = __("You are in class right now. What would you like to do?")
    #End Room Set-up
    
# Class Room Menu Start <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<        
    menu:
        "[Line]" 
        "Take the morning class" if Weekday < 5 and Current_Time == __("Morning"):
                if Round >= 30:
                    jump Take_Class
                else: 
                    "Class is already letting out. You can hang out until the next one."             
        "Take the afternoon class" if Weekday < 5 and Current_Time == __("Midday"):
                if Round >= 30:
                    jump Take_Class
                else: 
                    "Class is already letting out. You can hang out until they lock up for the night." 
        "There are no classes right now (locked)" if Weekday >= 5 or Current_Time == __("Evening") or Current_Time == __(__("Night")):
                pass
        ...
        "Lock the door" if "locked" not in Player.Traits:
                    if Current_Time == __("Evening") or Current_Time == __(__("Night")) or Weekday >=5:
                            "You lock the door"
                            $ Player.Traits.append("locked")
                            call Taboo_Level
                    else:
        ...
        "Wait" if Current_Time != __("Night"):
                ...
                if Current_Time == __("Midday"):
                            $ Line = __("A new class is in session. What would you like to do?")
                if Current_Time == __("Evening"):
                            $ Line = __("Classes have let out for the day. What would you like to do?")
...
label Take_Class:                       #Class events 
    ...
    if "class" in Player.DailyActions:
            $ Line = __("The session begins.")
    elif Round >= 80:
            $ Line = __("You make it in time for the start of the class.")
    elif Round >= 50:
            $ Line = __("You get in a bit late, but there's plenty of class left.")
    elif Round >= 30:
            $ Line = __("You're pretty late, but catch the tail end of the class.")
    $ Trigger = 0
    ...
    if RogueX in Present and D20 > 10 and RogueX.Inbt >= 500 and RogueX.Loc == "bg classroom":        
        ...
        $ Line = Line + renpy.random.choice([__(" It was fairly boring."),
                __(" It was a lesson in mutant biology."),
                __(" You took a math course."),
                __(" You watched a movie about sealions."),
                __(" That was fun."),
                __(" Applied trigonometry is surprisingly interesting, especially when Cyclops demonstrates using it for trick shots."),
                __(" Geopolitical science: Latveria to Madripoor."),
                __(" Today's lecture is on reading body language. You suppose if anyone would know about reading people. . ."),
                __(" The topic of the day is Mutants and the larger superhuman community."), 
                __(" Capes: What Your Name and Costume Say About You turns out to be pretty lively as you participate in a debate on costume designs."),
                __(" The topic is \"Mutants VS Mutates.\" As it turns out, the terms aren’t interchangeable."),  
                __(" Today's class is on how to present yourself to the public. She uses Spider-Man as an example of how bad PR makes your life harder than it needs to be."),                     
                __(" Mutant History, Apocalypse to Dark Phoenix."),
                __(" You spend some time learning about politics. Senator Trask seems like a real piece of work."),
                __(" You spend class learning about Aristotelian philosophy. Or about "+EmmaX.Name+"'s breasts."),
                __(" You learn how civil laws apply to mutant powers by studying some high profile case studies. It's surprisingly interesting."),
                __(" You listen as a guest speaker describes working with a Genosha-based NGO trying to rehabilitate mutants in the States."),
                __(" Today the teacher is describing the theory behind mutant powers. For some reason, you get the impression she is glancing at you during the lecture."),                                                                                                 
                __(" Game writing for dummies, eh?")]) 
        "[Line]"    
    ...
    call Set_The_Scene 
    call EventCalls
    $ Line = __("What would you like to do next?")
    jump Class_Room
...
#        extend ""        
        ...
        "Wait. (locked)" if Current_Time == __("Night"):
                    pass
        "Wait." if Current_Time != __("Night"):
                    "You hang out for a bit."
                    call Wait   
...
#        call Rogue_Frisky_Danger
    ...
    if Round >= 80:
            $ Line = __("You have a long session in the Danger Room.")
    elif Round >= 50:
            $ Line = __("You have a short workout in the Danger Room.")
    else:
            $ Line = __("You squeeze in a quick session in the Danger Room.")
    ...
    if D20 >= 18:
            ...
            $ Line = Line + renpy.random.choice([__(" It was fairly boring."), 
                    __(" You do some training with basic firearms."), 
                    __(" You run the obstacle course."),
                    __(" You fight in a simulated battle against the Brotherhood."),
                    __(" You help take down a holographic Sentinel."),
                    __(" You take part in a training exercise against the Avengers. As if the X-Men and Avengers would ever fight."),                     
                    __(" You and some of the others take part in a survival exercise. . . also known as \"try to last as long as you can while Wolverine hunts you down one by one.\""),
                    __(" You decide to test yourself by facing off against Magneto solo. It goes about as well as you’d expect."),
                    __(" You use the Danger Room’s holograms to relive some of the original X-Men’s biggest battles. You learn quite a bit about teamwork."), 
                    __(" Beast is teaching a class on parkour. You take part and pick up a few pointers. You’re no Spider-Man, but at least you pick up a few things."),  
                    __(" You participate in an emergency drill. You pick up quite a few tips about first aid, triage and the proper way to move injured people."), 
                    __(" You take part in an urban emergency situation exercise. Cyclops takes the time to explain to you how to use cover to get close enough to use your powers."), 
                    __(" You take part in a jungle simulation exercise under Wolverine. You learn some basic survival techniques, but you privately hope you never need them."), 
                    __(" Your team fight a simulation of Magneto.")])   
            "[Line]"    
    if RogueX.Loc == bg_current:
    ...
    call Girls_Location 
    call Set_The_Scene
    $ Line = __("The training session has ended, what would you like to do next?")
...
# Pool Menu Start <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
    menu:
        ...
        "Want to swim?":
            if Current_Time == __("Night") and AloneCheck():
...
label SwimSuit(BO=[]):
        ...
        while BO:                       
                if BO[0].Loc == bg_current and BO[0].Swim[0] and BO[0] not in Party and BO[0].Schedule[Weekday][Time_Count] == "bg pool": 
                        ...
                        $ BO[0].Outfit = "swimwear" # puts on her swimsuit
                        $ BO[0].OutfitChange() # puts on her swimsuit
                $ BO.remove(BO[0])
        return
                ...
                if renpy.random.randint(1, 20) < 5:
                        ...
                        if Nearby:        
                                ...
                                if len(Nearby) > 1:
                                        $ Nearby[1].Loc = "nearby" 
                                $ Nearby[0].Loc = "nearby" 
        "Wait.  [[no time](locked)" if Current_Time == __("Night"):
                pass
                    ...
                    if Line:                        
                        ...
                        while BO:
                                ...
                                if BO[0].Loc == bg_current and BO[0] not in StayCount:                     
                                        ...
                                        elif Line == __("spot") and ApprovalCheck(BO[0], 1000, __("LI"),Alt=[[KittyX],1200]):   #1200 for Kitty?
                                            $ StayCount.append(BO[0]) 
                                        elif Line == __("watch you") and ApprovalCheck(BO[0], 600, __("O"),Alt=[[EmmaX],500]):   #500 for Emma?
                                            $ StayCount.append(BO[0]) 
...
#/ / Showering begins / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / /
    ...
    if StayCount:
                ...
                #solo shower
                $ Line = __("You take a quick shower") + renpy.random.choice([__(". It was fairly uneventful."), 
                        __(". A few people came and went as you did so."), 
                        __(". That was refreshing.")])  
                "[Line]"    
...
label Shower_Sex(Options=0,Line=0):
        ...
        if len(StayCount) > 1 and D20 >= 10:
                ...
                $ Line = renpy.random.choice([__("It was fairly uneventful."), 
                    __("A few people came and went as you did so."), 
                    __("That was refreshing.")]) 
                "[Line]"
                if len(StayCount) > 1: 
...
label Study_Room_Entry:  
    ...
    menu:
            ...
            "Use the key to enter" if Current_Time == __("Night") and "Xavier" in Keys:
                    "You use your key."
                    $ Line = 0
            
            "Ask Kitty" if Current_Time == __("Night") and KittyX in Party:
                    $ Line = __("kitty")
...
# Study Room Menu Start <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    if Current_Time == __("Night"):
        $ Line = __("You are in Xavier's Study, but he isn't in at the moment. What would you like to do?")
    else:
        $ Line = __("You are in Xavier's Study. What would you like to do?")
    menu:
        "[Line]"        
        "Chat" if Current_Time == __("Night"): #fix, open up once sex while in office is fine
                    call Chat
        
        "Plan Omega!" if Current_Time != __("Night") and RogueX.Loc == bg_current:
                    if ApprovalCheck(RogueX, 1500, TabM=1, Loc="No") and Player.Lvl >= 5:
                        call Xavier_Plan(RogueX) #Plan_Omega
                    else:
                        ch_r "What?"
        "Plan Kappa!" if Current_Time != __("Night") and KittyX.Loc == bg_current:
                    if "Xavier's photo" in Player.Inventory and Player.Lvl >= 5 and ApprovalCheck(KittyX, 1500, TabM=1, Loc="No"):                   
                        ...
                        ch_k "What?"     
        "Plan Psi!" if Current_Time != __("Night") and EmmaX.Loc == bg_current:
                    if ApprovalCheck(EmmaX, 1500, TabM=1, Loc="No") and Player.Lvl >= 5:
                        call Xavier_Plan(EmmaX) #Plan_Psi
                    else:
                        ch_e "What?"  
        "Plan Chi!" if Current_Time != __("Night") and LauraX.Loc == bg_current:
                    if LauraX.Lvl >= 2 and ApprovalCheck(LauraX, 1500, TabM=1, Loc="No") and ApprovalCheck(LauraX, 750, "I"):   
                        ...
                        ch_l "Huh?"                  
        "Explore" if Current_Time == __("Night") and "explore" not in Player.RecentActions: 
                    $ Cnt = 0    
                    $ Player.RecentActions.append("explore")
                    jump Study_Room_Explore
            
        "Wait":
                    if Current_Time == __(__("Night")):
                            "You probably don't want to be here when Xavier gets in."
                    elif Current_Time == __("Evening"):
...
label Girls_Room_Entry(Chr=0):
        ...
        if Chr in Party:
                        if Current_Time == __(__("Night")) or (Current_Time == __("Evening") and Round <= 10):                         
        ...
        if Round >= 10 and Chr.Loc == bg_current and "gonnafap" in Chr.DailyActions and D20 >= 5: 
                ...
                if Line != "knock" and Chr in Keys: 
                        if Chr.Loc == bg_current:
                            ...
                            if Round <= 10:        #add "no" condtion here
                                    ...
                                    #girl caught fapping  
                                    call Girl_Caught_Mastubating(Chr) 
                            elif D20 >=15 and (Current_Time == __("Night") or Current_Time == __("Morning")):                          
                ...
                if Chr.Loc != bg_current:
                        ...
                        $ bg_current = "bg campus"   
                        jump Misplaced 
                elif Current_Time == __("Night") and "noentry" in Chr.RecentActions:                
                        if Chr == RogueX:
                        ...
                        $ bg_current = "bg campus"   
                        jump Misplaced 
                elif Current_Time == __("Night") and (Chr.Sleep or Chr.SEXP >= 30):                                                   
                        #It's late but she really likes you                    
                        if Chr == RogueX:
                ...
                elif Current_Time == __("Night") and (ApprovalCheck(Chr, 1000, "LI") or ApprovalCheck(Chr, 600, "OI")):     
                        #It's late but she really likes you
                        if Chr == RogueX:
                                ch_r "It's pretty late, [Chr.Petname], but you can come in for a little bit." 
                        elif Chr == KittyX:
                ...
                elif Current_Time == __("Night") and (ApprovalCheck(Chr, 500, "LI") or ApprovalCheck(Chr, 300, "OI")):
                        if Chr == RogueX:
...
# [RogueX.Name]'s Room Menu Start <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    if RogueX.Loc == bg_current:
        $ Line = __("You are in "+RogueX.Name+"'s room. What would you like to do?")
    else:
        $ Line = __("You are in "+RogueX.Name+"'s room, but she isn't here. What would you like to do?")
    menu:
        "[Line]"
...
# [KittyX.Name]'s Room Menu Start <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    if KittyX.Loc == bg_current:
        $ Line = __("You are in "+KittyX.Name+"'s room. What would you like to do?")
    else:
        $ Line = __("You are in "+KittyX.Name+"'s room, but she isn't here. What would you like to do?")
    menu:
        "[Line]"
        ...
        "Sleep." if Current_Time == __("Night") and KittyX.Loc == bg_current:
                    call Round10
                    call Girls_Location
                    call EventCalls 
...
# Emma's Room Menu Start <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    if EmmaX.Loc == bg_current:
        $ Line = __("You are in "+EmmaX.Name+"'s room. What would you like to do?")
    else:
        $ Line = __("You are in "+EmmaX.Name+"'s room, but she isn't here. What would you like to do?")
    menu:
        "[Line]"
        ...
        "Sleep." if Current_Time == __("Night") and EmmaX.Loc == bg_current:
                    call Round10
                    call Girls_Location
                    call EventCalls 
...
# Laura's Room Menu Start <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    if LauraX.Loc == bg_current:
        $ Line = __("You are in "+LauraX.Name+"'s room. What would you like to do?")
    else:
        $ Line = __("You are in "+LauraX.Name+"'s room, but she isn't here. What would you like to do?")
    menu:
        "[Line]"
